...
- **Teleop view_mode**: Use `--view_mode rerun` for LeRobot Rerun camera feeds, or `--view_mode mujoco` (default) for the MuJoCo GLFW window. Both modes record the same LeRobot v3.0 dataset. Keyboard recording controls are identical in both modes: Left/R cancel & rerecord, Right/N save & next, ESC/Q stop. Joy-Con and leader use LeRobot n/r/q or device buttons.
- **Episode reset (sim)**: `robot.reset_mode: auto` (default) resets arm home + cube layout from `cube_positions.json` before each recorded episode. Use `manual` for real-hardware-style recording. `dataset.reset_time_s` still allows a short teleop window between episodes.
