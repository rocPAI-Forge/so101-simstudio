#!/bin/bash
# Create/upload the Lab 01 SmolVLA Hub repo (MI300X bs64 @ 50K pretrained_model).
#
# Usage (from repo root, after hf auth login):
#   ./labs/lab01_pnp/push_smolvla_model.cmd
#   ./labs/lab01_pnp/push_smolvla_model.cmd --card-only
#
# Uses the current HF_ENDPOINT (this host: hf-mirror.com). Force official Hub:
#   LAB01_HF_UPLOAD_ENDPOINT=https://huggingface.co ./labs/lab01_pnp/push_smolvla_model.cmd
#
# Override:
#   LAB01_SMOLVLA_HF_REPO_ID=your-user/your-repo ./labs/lab01_pnp/push_smolvla_model.cmd
#   LAB01_POLICY_PATH=./outputs/train/.../pretrained_model ./labs/lab01_pnp/push_smolvla_model.cmd
set -euo pipefail
...
CARD="$_LAB01_DIR/hf_model_card_smolvla.md"
REPO_ID="${LAB01_SMOLVLA_HF_REPO_ID:-alexhegit/so101-simstudio-lab01-pnp-smolvla}"
CKPT_STEP="${LAB01_SMOLVLA_CKPT_STEP:-050000}"
POLICY_PATH="${LAB01_POLICY_PATH:-$LAB01_TRAIN_OUTPUT/checkpoints/${CKPT_STEP}/pretrained_model}"
...
echo "=== Lab 01: push SmolVLA model ==="
...
if [[ "$CARD_ONLY" != true ]]; then
    ...
    "$HF" upload "$REPO_ID" "$POLICY_PATH" . \
        --repo-type model \
        --commit-message "Add SmolVLA MI300X bs64 50K pretrained_model"
fi

echo "=== Upload model card ==="
"$HF" upload "$REPO_ID" "$CARD" README.md \
    --repo-type model \
    --commit-message "Update SmolVLA model card (SimStudio Lab 01)"
