#!/bin/bash
# Create/upload the Lab 01 ACT 6-D Hub repo (MI300X 50K, joint-pos state).
#
...
#   ./labs/lab01_pnp/push_act_state6_model.cmd --card-only
#
# Override:
#   LAB01_ACT_STATE6_HF_REPO_ID=your-user/your-repo ./labs/lab01_pnp/push_act_state6_model.cmd
#   LAB01_ACT_STATE6_POLICY_PATH=./outputs/train/.../pretrained_model ./labs/lab01_pnp/push_act_state6_model.cmd
set -euo pipefail
...
CARD="$_LAB01_DIR/hf_model_card_act_state6.md"
REPO_ID="${LAB01_ACT_STATE6_HF_REPO_ID:-alexhegit/so101-simstudio-lab01-pnp-act-state6}"
CKPT_STEP="${LAB01_ACT_STATE6_CKPT_STEP:-050000}"
# Do not reuse LAB01_POLICY_PATH from _env.sh (that default is SmolVLA eval).
POLICY_PATH="${LAB01_ACT_STATE6_POLICY_PATH:-./outputs/train/lab01_pnp_act_state6/checkpoints/${CKPT_STEP}/pretrained_model}"
...
echo "=== Lab 01: push ACT 6-D model ==="
...
if [[ "$CARD_ONLY" != true ]]; then
    ...
    "$HF" upload "$REPO_ID" "$POLICY_PATH" . \
        --repo-type model \
        --commit-message "Add ACT 6-D joint-state MI300X 50K pretrained_model"
fi

echo "=== Upload model card ==="
"$HF" upload "$REPO_ID" "$CARD" README.md \
    --repo-type model \
    --commit-message "Update ACT 6-D model card (SimStudio Lab 01)"
